...
# Parses the formula from a given file path
...
# Parses a formula given in DIMACS as string
...
class Formula:
    ...
    def __init__(self, num_vars: int, clauses: list[set], debug: bool = False):
        ...
        self.assignment = {}  # uses absolute value
        self.open_assignments = set(
            range(1, num_vars + 1)
        )  # number of not yet assigned vars
        self.clauses_of_watched_literal = {
            i: set() for i in range(-num_vars, num_vars + 1) if i != 0
        }  # maps literal to clauses where it's watched
        self.watched_literals_in_clause = (
            {}
        )  # maps clause to literals that are watchd in this clause
        self.bcp_queue = []
        self.trail = []  # stores indices of assignment when decision
        self.debug = debug  # prints debug information

    def __str__(self):
        return (
            "Clauses: ["
            + ", ".join(
                ["{" + " ".join(map(str, clause)) + "}" for clause in self.clauses]
            )
            + "] \n"
            + "Assignment: ["
            + " ,".join(f"{var}={value}" for (var, value) in self.assignment.items())
            + "]"
        )

    # Sets the value of a variable, removes all clauses which then are true
    # and removes the variable where it occurs negated
    # NOTE: only used in simple_recursive
    ...
    def _assign_value(self, var: int, value: bool):
        ...
        self.assignment[abs(var)] = (
            value if var > 0 else (not value)
        )  # store truth value
    ...
    # adds the decision to the queue and calls bcp and returning its result
    ...
    def bcp(self):
        ...
        while self.bcp_queue:  # while not_empty()
            ...
            for i in list(
                self.clauses_of_watched_literal.get(-unit_lit, [])
            ):  # check clauses where lit is negated
                ...
                if self.debug:
                    print(
                        f"clause: {clause} | without watched: {clause - self.watched_literals_in_clause[i]}"
                    )

                for lit in (
                    clause - self.watched_literals_in_clause[i]
                ):  # iterate over non-watched literals in clause
                    assigned = self.assignment.get(
                        abs(lit)
                    )  # get assignment of current lit
                    if self.debug:
                        print(f"lit:{lit}")
                    if assigned is None or assigned == (
                        lit > 0
                    ):  # literal is unassigned or true
                        if (
                            i not in self.clauses_of_watched_literal[lit]
                        ):  # literal is not watched by other watcher
                ...
                if (
                    i in self.clauses_of_watched_literal[-unit_lit]
                ):  # if watch pointer was not moved
                    ...
                    if self.debug:
                        print(
                            f"can be switched to {self.watched_literals_in_clause[i] - {-unit_lit}}"
                        )
                    if not self.watched_literals_in_clause[i] - {-unit_lit}:
                        return False

                    other_watched_literal = next(
                        iter(self.watched_literals_in_clause[i] - {-unit_lit})
                    )
                    if self.debug:
                        print(f"Other watched lit: {other_watched_literal}")

                    assigned = self.assignment.get(
                        abs(other_watched_literal)
                    )  # get assignment of current lit
                    if (
                        assigned is None
                    ):  # other lit not yet assigned, assign True and add to queue
                        ...
                        self.bcp_queue = (
                            []
                        )  # Resets the bcp queue if conflict was derived
                        return False  # conflict derived
        # if no conflict was derived
        return True


# Returns True if formula is SAT, False otherwise
# uses a naiive recursive implementation
def simple_recursive(formula: Formula):
    ...
    if set() in formula.clauses:  # contains empty clause
        ...
        var = next(
            iter(formula.clauses[0])
        )  # gets the first variable in the first clause
        return simple_recursive(formula.set_value(var)) or simple_recursive(
            formula.set_value(-var)
        )


# Returns True if formula is SAT, False otherwise
# uses a non-recursive implementation using BCP with watched literals
def simple_non_recursive(formula: Formula):

    decisions = []  # stores decisions made
    ...
    decisions.append(
        0 + len(formula.assignment.keys())
    )  # first decision level (index of first decision in trail), to ignore first bcp assignments
    val = True
    var = next(iter(formula.open_assignments))

    while formula.open_assignments:  # not empty
        # checks if decision leads to conflict
        if formula.decide(var, val):

            if not formula.open_assignments:  # empty after bcp
                return True

            # No conflict, so we assign next variable
            decisions.append(len(formula.assignment.keys()))
            var = next(iter(formula.open_assignments))
            val = True

        else:  # Revert the current assignment
            # Nothing to revert -> un-SAT
            if len(decisions) == 0:
                return False
            start = decisions.pop()
            ...
            #     start = 0
            #
            assignments_to_revert = formula.trail[
                start:
            ]  # all assigments starting from last decision
            var = assignments_to_revert[0]
            val = False

            # removes them from the trail
            del formula.trail[start:]
            for (
                lit
            ) in (
                assignments_to_revert
            ):  # revert all assignments related to this decision
...
def main():
    ...
    parser.add_argument(
        "--recursive",
        metavar="FILE",
        help="Use recursive solver to solve formula given in DIMACS.",
    )
    parser.add_argument(
        "--non-recursive",
        metavar="FILE",
    ...
    if args.recursive:
        ...
        print(
            "Please use --recursive <path_to_file> or --non-recursive <path_to_file>."
        )
